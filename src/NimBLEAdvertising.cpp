...
void NimBLEAdvertising::setServiceData(const NimBLEUUID& uuid, const std::string& data) {
    switch (uuid.bitSize()) {
        case 16: {
            std::vector<uint8_t>((uint8_t*)&uuid.getNative()->u16.value, (uint8_t*)&uuid.getNative()->u16.value + 2).swap(m_svcData16);
        ...
        case 32: {
            std::vector<uint8_t>((uint8_t*)&uuid.getNative()->u32.value, (uint8_t*)&uuid.getNative()->u32.value + 4).swap(m_svcData32);
        ...
        case 128: {
            std::vector<uint8_t>(uuid.getNative()->u128.value, uuid.getNative()->u128.value + 16).swap(m_svcData128);
...
void NimBLEAdvertising::setScanFilter(bool scanRequestWhitelistOnly, bool connectWhitelistOnly) {
    NIMBLE_LOGD(LOG_TAG,
                ">> setScanFilter: scanRequestWhitelistOnly: %d, connectWhitelistOnly: %d",
...
void NimBLEAdvertising::setAdvertisementData(NimBLEAdvertisementData& advertisementData) {
    ...
    int rc =
        ble_gap_adv_set_data((uint8_t*)advertisementData.getPayload().data(), advertisementData.getPayload().length());
    if (rc != 0) {
...
void NimBLEAdvertising::setScanResponseData(NimBLEAdvertisementData& advertisementData) {
    ...
    int rc = ble_gap_adv_rsp_set_data((uint8_t*)advertisementData.getPayload().data(),
...
# endif
    ...
    if (!m_customAdvData && !m_advDataSet) {
        ...
        if (m_advData.mfg_data_len > 0) payloadLen += (2 + m_advData.mfg_data_len);

        if (m_advData.svc_data_uuid16_len > 0) payloadLen += (2 + m_advData.svc_data_uuid16_len);

        if (m_advData.svc_data_uuid32_len > 0) payloadLen += (2 + m_advData.svc_data_uuid32_len);

        if (m_advData.svc_data_uuid128_len > 0) payloadLen += (2 + m_advData.svc_data_uuid128_len);

        if (m_advData.uri_len > 0) payloadLen += (2 + m_advData.uri_len);

        if (m_advData.appearance_is_present) payloadLen += (2 + BLE_HS_ADV_APPEARANCE_LEN);

        if (m_advData.tx_pwr_lvl_is_present) payloadLen += (2 + BLE_HS_ADV_TX_PWR_LVL_LEN);

        if (m_advData.slave_itvl_range != nullptr) payloadLen += (2 + BLE_HS_ADV_SLAVE_ITVL_RANGE_LEN);
        ...
        for (auto& it : m_serviceUUIDs) {
            if (it.getNative()->u.type == BLE_UUID_TYPE_16) {
                ...
                if (nullptr ==
                    (m_advData.uuids16 = (ble_uuid16_t*)realloc((void*)m_advData.uuids16,
            ...
            if (it.getNative()->u.type == BLE_UUID_TYPE_32) {
                ...
                if (nullptr ==
                    (m_advData.uuids32 = (ble_uuid32_t*)realloc((void*)m_advData.uuids32,
            ...
            if (it.getNative()->u.type == BLE_UUID_TYPE_128) {
                ...
                if (nullptr == (m_advData.uuids128 =
                                    (ble_uuid128_t*)realloc((void*)m_advData.uuids128,
        ...
        if (m_scanResp && !m_customScanResponseData) {
            ...
            switch (rc) {
                ...
                default:
                    NIMBLE_LOGE(LOG_TAG,
                                "Error setting scan response data; rc=%d, %s",
                                rc,
        ...
        if (rc == 0) {
            ...
            switch (rc) {
                ...
                default:
                    NIMBLE_LOGE(LOG_TAG,
                                "Error setting advertisement data; rc=%d, %s",
                                rc,
                                NimBLEUtils::returnCodeToString(rc));
                    break;
            }
 ...
 */
void NimBLEAdvertisementData::setServices(const bool complete, const uint8_t size, const std::vector<NimBLEUUID>& v_uuid) {
    char cdata[2];
