...
size_t NimBLEClient::deleteService(const NimBLEUUID& uuid) {
    ...
    auto it = m_servicesVector.begin();
...
# else
        rc = ble_gap_connect(NimBLEDevice::m_own_addr_type,
                             &peerAddr_t,
                             m_connectTimeout,
                             &m_pConnParams,
                             NimBLEClient::handleGapEvent,
 ...
 */
int NimBLEClient::serviceDiscoveredCB(uint16_t                     conn_handle,
...
int NimBLEClient::handleGapEvent(struct ble_gap_event* event, void* arg) {
    ...
    switch (event->type) {
        case BLE_GAP_EVENT_DISCONNECT: {
            ...
            if (!pClient->m_connEstablished) break;
        ...
        case BLE_GAP_EVENT_NOTIFY_RX: {
            if (pClient->m_conn_id != event->notify_rx.conn_handle) return 0;
            ...
            for (auto& it : pClient->m_servicesVector) {
                ...
                if (characteristic != cVector->cend()) {
                    ...
                    if ((*characteristic)->m_notifyCallback != nullptr) {
                        NIMBLE_LOGD(LOG_TAG,
                                    "Invoking callback for notification on characteristic %s",
                                    (*characteristic)->toString().c_str());
                        (*characteristic)
                            ->m_notifyCallback(*characteristic,
        ...
        case BLE_GAP_EVENT_PASSKEY_ACTION: {
            ...
            if (pClient->m_conn_id != event->passkey.conn_handle) return 0;
