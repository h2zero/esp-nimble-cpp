 ...
 */
NimBLEClient* NimBLEDevice::getClientByID(uint16_t connHandle) {
    for (const auto clt : m_pClients) {
...
NimBLEAddress NimBLEDevice::getAddress() {
    ble_addr_t addr{};
    if (ble_hs_id_copy_addr(BLE_ADDR_PUBLIC, addr.val, NULL) == BLE_HS_ENOADDR) {
...
bool NimBLEDevice::setMTU(uint16_t mtu) {
    NIMBLE_LOGD(LOG_TAG, ">> setMTU: %d", mtu);
...
void NimBLEDevice::setScanDuplicateCacheSize(uint16_t cacheSize) {
    ...
    NIMBLE_LOGD(LOG_TAG, "Set duplicate cache size to: %u", cacheSize);
 ...
 * If clearAll is false, the objects will remain and can be used again after reinitializing the stack.
...
bool NimBLEDevice::isIgnored(const NimBLEAddress& address) {
    for (const auto& it : m_ignoreList) {
...
void NimBLEDevice::removeIgnored(const NimBLEAddress& address) {
    auto it = std::find(m_ignoreList.begin(), m_ignoreList.end(), address);
